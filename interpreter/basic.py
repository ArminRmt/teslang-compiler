...
class Lexer:
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in " \t":
                ...
                tokens.append(
                    Token(TT_PLUS, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "-":
                ...
                tokens.append(
                    Token(TT_MUL, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "/":
                tokens.append(
                    Token(TT_DIV, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "^":
                tokens.append(
                    Token(TT_POW, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "(":
                tokens.append(
                    Token(TT_LPAREN, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == ")":
                tokens.append(
                    Token(TT_RPAREN, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "[":
                tokens.append(
                    Token(TT_LSQUARE, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "]":
                tokens.append(
                    Token(TT_RSQUARE, value=self.current_char, pos_start=self.pos)
                )
                self.advance()
            elif self.current_char == "!":
                (
                ...
                )  # if = comes after ! we have not equal otherwise throw error
                if error:
                    return [], error
                tokens.append(token)
            elif self.current_char == "=":
    ...
    def make_string(self):
        ...
        self.advance()  # end token is " and we advance
...
class ListNode:
    def __init__(self, element_nodes, pos_start, pos_end):
        self.element_nodes = element_nodes  # string or number
...
#######################################
# PARSE RESULT
# # takes a token string as input and with the help of existing grammar, converts it into the corresponding Intermediate Representation(IR)
#######################################
...
class Parser:
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "VAR"):
            res.register_advancement()  # advance to the next token
            self.advance()
...
#######################################
# RUNTIME RESULT
# same as parse result class as keep track of result  and show error if there is any
#######################################
...
#######################################
# CONTEXT
# function that we are inside or mudle(hole program)
#######################################
...
class SymbolTable:
    def __init__(self, parent=None):
        self.symbols = {}
        self.parent = parent  # global symbol table with global variables in the code that can acssed anywhere in code

    def get(self, name):
        value = self.symbols.get(name, None)
        if (
            value == None and self.parent
        ):  # self.parent (global symbol table wont have any parent becaues its a root)
...
class Interpreter:
    # procces node and visit all child node
    ...
    def visit_VarAccessNode(self, node, context):
        ...
        if not value:  # variable is not defined yet
            return res.failure(
                RTError(
                    node.pos_start,
                    node.pos_end,
