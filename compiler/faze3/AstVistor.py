...
# A simple thing to get your idea my dear friend
...
# The AstVisitor class defines an abstract syntax tree visitor,
# which is used to traverse and manipulate the AST nodes.
# generated by the PLY parser.
# This class provides methods for executing the AST and generating code from it.
...
class AstVistor:
    ...
    def execute(self):
        ...
        # This function searches for a function symbol with the given name in the idenInfo list.
        # It returns the symbol object if found

        def find_symbol(name, count):
            symbol = find_function_symbol(name)
            if symbol:
                return symbol
            symbol = find_argument_symbol(name)
            if symbol:
                return symbol
        ...
        if self.action == "function":
            ...
            # Generate the code for function intermidiate code representation
            ...
            if flag:
                ...
                # If the name doesn't already exist, create a new symbol table entry for the function.
                ...
                # Add the function's parameters to the symbol table entry.
                if f_args:
            ...
            # Generate the intermidiate code for return statement
            add_code = f"ret\n"
            self.codegen.add_code(add_code)

            # Extract the name and return type of the function from the parser stack.
            for item in p_stack:
            ...
            # Check if the function return value is a constant or a variable.
            # If it's a constant, set the return type accordingly
            # If it's a variable, look up the symbol in the symbol table.
            ...
            # If the condition is true, generate code for the statement.
            # Otherwise, if there is an else statement, generate code for that instead.
            # it is optimezed based on if if and else are fales so no need for resiter allocation
            if p_cond:
                if_label = self.codegen.get_label()
                self.codegen.add_code(f"{if_label}:\n")
                self.codegen.add_code(f"jz {if_label}\n")
                result = p_stmt

            elif len(self.params) > 2:
                else_label = self.codegen.get_label()
                self.codegen.add_code(f"{else_label}:\n")
                self.codegen.add_code(f"jz {else_label}\n")
                p_else_stmt = self.params[2]
                result = p_else_stmt

        elif self.action == "while":
            while_cond, while_stmt = self.params[0], self.params[1]
            loop_label = self.codegen.get_label()
            end_label = self.codegen.get_label()

            self.codegen.add_code(f"{loop_label}:\n")

            # while while_cond:
            #     result = while_stmt
            #    # Jump back to the loop label to check the condition again
            #     self.codegen.add_code(f"jmp {loop_label}\n")

            # Jump to the end label if the condition is false
            self.codegen.add_code(f"jz {end_label}\n")
            ...
            # Find the name of the function from the parsing stack.
            ...
            if var is None:
                ...
                if type(value_to_assign).__name__ != var.var_type:
                    ...
                    # If the assigned value is of the correct type, update the symbol table entry with the assigned value.
            ...
            # the move and iget shouldn't used in same time in some cases so i use this flag
            # If move_iget_flag is set, move the value of the variable to a temporary register.
            ...
            # Count the number of function scopes to avoid conflicts with other variables.
            count = 0
            ...
            # If the symbol is an integer, print its value.
            # If the symbol is Object, print its value attribute.
            ...
            # Initialize a new list with the specified length.
            result = [None for _ in range(list_length)]

        elif self.action == "builtin_scan":
            # Disable move_iget_flag for conflict of mov and scan in intermidiate code
            ...
            if array_node is None:
                ...
                # If the symbol is an array, assign the new value to the specified index.
        ...
        # expr[expr]
        ...
        elif self.action == "ListNode":  # TODO what if string also be in list near int
            ...
            # register to store the return value (intermidiate code part)
            ...
            # TODO this part is for assigning function argumans the argumans that used in function call
            ...
            # TODO and op in "+-*/%**><=" and type(a) == type(b) and type(a) in [int, float] and type(b) in [int, float]
